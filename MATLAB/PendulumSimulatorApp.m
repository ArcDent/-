function PendulumSimulatorApp
    ...
    % 创建输入面板
    panel = uipanel(fig, 'Title', 'Parameters', 'Position', [20, 420, 300, 250]);
    
    % 添加输入控件
    uilabel(panel, 'Text', 'R:', 'Position', [20, 200, 100, 22]);
    editR = uieditfield(panel, 'numeric', 'Value', 0.5, 'Position', [120, 200, 100, 22]);
    
    uilabel(panel, 'Text', 'L:', 'Position', [20, 170, 100, 22]);
    editL = uieditfield(panel, 'numeric', 'Value', 1.0, 'Position', [120, 170, 100, 22]);
    
    uilabel(panel, 'Text', 'g:', 'Position', [20, 140, 100, 22]);
    editg = uieditfield(panel, 'numeric', 'Value', 9.8, 'Position', [120, 140, 100, 22]);
    
    uilabel(panel, 'Text', 'a₀:', 'Position', [20, 110, 100, 22]);
    edita0 = uieditfield(panel, 'numeric', 'Value', 1.0, 'Position', [120, 110, 100, 22]);
    
    uilabel(panel, 'Text', 'ω:', 'Position', [20, 80, 100, 22]);
    editomega = uieditfield(panel, 'numeric', 'Value', 2.0, 'Position', [120, 80, 100, 22]);
    ...
    btn = uibutton(fig, 'push', 'Text', 'Simulate', 'Position', [120, 380, 100, 30],...
        'ButtonPushedFcn', @(btn,event) runSimulation(fig, editR, editL, editg, edita0, editomega, editTf));
...
function runSimulation(fig, editR, editL, editg, edita0, editomega, editTf)
    % 获取参数值
    R = editR.Value;
    L = editL.Value;
    g = editg.Value;
