function LatoLato_Simulation
    ...
    function runSimulation(~, ~)
        ...
        % 主求解循环
        while t0 < T_max
            [t, y, te, ye, ie] = ode45(@(t,y) pendulumODE(t, y, denom, g, a0, omega), ...
                                       [t0, T_max], y0, options);
        ...
        ylim(ax2, [min(theta_dot)*1.1, max(theta_dot)*1.1]);
    ...
    % 摆的微分方程
    function dydt = pendulumODE(t, y, denom, g, a0, omega)
        theta = y(1);
        % theta_dot = y(2); % 未使用
        
        % 计算系数
        coeff = (3*a0*omega^2)/denom * cos(omega*t) + (3*g)/denom;
